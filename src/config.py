...
class Augmentations(Validation, validate_assignment=True):
    ...
    motion_blur_prob: float = 0.0  # Probability of applying motion blur, 0 - no blur
    motion_blur_kernel_size: int | list[int] = [3, 7]  # Motion blur kernel size range
    ...
    color_jitter_saturation: float = 0.0  # Saturation jitter factor, 0 - no saturation jitter
    color_jitter_hue: float = 0.0  # Hue jitter factor, 0 - no hue jitter
    hsv_prob: float = 0.0  # Probability of HSV augmentation
    hsv_hue_shift: int = 20  # Hue shift range [-hsv_hue_shift, +hsv_hue_shift]
    hsv_sat_shift: int = 30  # Saturation shift range
    hsv_val_shift: int = 20  # Value shift range
    
    # === Compression Augmentations (Phase 1 - Highest Priority) ===
    jpeg_quality: int | list[int] = [40, 100]  # JPEG quality range, 100 - no JPEG compression
    video_compression_prob: float = 0.0  # Probability of simulating video compression artifacts
    video_compression_quality: int | list[int] = [20, 50]  # Video compression quality range
    
    # === Downscale Augmentations (Phase 1) ===
    downscale_prob: float = 0.0  # Probability of downscaling
    downscale_scale: float | list[float] = [0.5, 0.9]  # Downscale factor range
    downscale_interpolation: int | list[int] = 2  # Interpolation: 0=nearest, 2=bilinear, 3=bicubic
    
    # === Noise Augmentations (Phase 2) ===
    gaussian_noise_sigma: float = 0.0  # Standard deviation of Gaussian noise to add, 0 - no noise
    iso_noise_prob: float = 0.0  # Probability of ISO noise (camera noise)
    iso_noise_color_shift: float = 0.05  # Color shift for ISO noise
    iso_noise_intensity: float | list[float] = [0.1, 0.5]  # ISO noise intensity range
    multiplicative_noise_prob: float = 0.0  # Probability of multiplicative noise
    multiplicative_noise_multiplier: float | list[float] = [0.9, 1.1]  # Multiplier range
    
    # === Dropout Augmentations (Phase 4) ===
    coarse_dropout_prob: float = 0.0  # Probability of coarse dropout
    coarse_dropout_max_holes: int = 8  # Maximum number of holes
    coarse_dropout_max_height: int = 32  # Maximum height of holes
    coarse_dropout_max_width: int = 32  # Maximum width of holes
    grid_mask_prob: float = 0.0  # Probability of grid mask
    grid_mask_ratio: float = 0.6  # Ratio of grid mask
    cutmix_prob: float = 0.0  # Probability of CutMix (applied in dataset/batch level)
    cutmix_alpha: float = 1.0  # Beta distribution alpha for CutMix
    
    # === Real-Fake Mixup (Phase 5 - DFDC 1st Place Strategy) ===
    real_fake_mixup_prob: float = 0.0  # Probability of Real-Fake Mixup
    real_fake_mixup_alpha: float = 0.5  # Beta distribution alpha (0.5 = more balanced mix)
    
    # === Advanced Augmentations ===
    auto_augment_prob: float = 0.0  # Probability of AutoAugment policy
    auto_augment_policy: str = "imagenet"  # AutoAugment policy: imagenet, cifar10, svhn
    
    # === Frequency Domain Augmentations (Phase 6 - SOTA Methods) ===
    dct_dropout_prob: float = 0.0  # Probability of DCT dropout (frequency domain)
    dct_dropout_channels: int | list[int] = [0, 1, 2]  # Channels to apply DCT dropout (0=Y, 1=Cb, 2=Cr)
    dct_dropout_ratio: float | list[float] = [0.1, 0.3]  # Ratio of DCT coefficients to drop
    high_pass_prob: float = 0.0  # Probability of high-pass filter
    high_pass_cutoff: float | list[float] = [0.1, 0.3]  # High-pass filter cutoff frequency
    low_pass_prob: float = 0.0  # Probability of low-pass filter
    low_pass_cutoff: float | list[float] = [0.5, 0.8]  # Low-pass filter cutoff frequency
    
    # === Geometric Transform (Phase 7) ===
    elastic_prob: float = 0.0  # Probability of elastic transform
    elastic_alpha: float | list[float] = [30, 50]  # Elastic transform alpha (deformation strength)
    elastic_sigma: float | list[float] = [4, 6]  # Elastic transform sigma (smoothness)
    
    # === Advanced Color Augmentations (Phase 7) ===
    gamma_prob: float = 0.0  # Probability of random gamma correction
    gamma_limit: float | list[float] = [0.7, 1.5]  # Gamma range
    clahe_prob: float = 0.0  # Probability of CLAHE (Contrast Limited Adaptive Histogram Equalization)
    clahe_clip_limit: float = 2.0  # CLAHE clip limit
    clahe_tile_grid_size: int | list[int] = [8, 8]  # CLAHE tile grid size
    posterize_prob: float = 0.0  # Probability of posterization (reduce color bits)
    posterize_bits: int | list[int] = [4, 7]  # Number of bits to keep (1-8)
    
    # === Self-Blended Images (Phase 8 - CVPR 2022 SOTA) ===
    sbi_prob: float = 0.0  # Probability of Self-Blended Images augmentation
    sbi_alpha_range: float | list[float] = [0.3, 0.7]  # Blending alpha range
    sbi_mask_type: str = "random"  # Mask type: random, half, quarter, grid
    
    # === Advanced Geometric & Lighting (Phase 8) ===
    perspective_prob: float = 0.0  # Probability of perspective transform
    perspective_distortion: float = 0.2  # Distortion scale (0.0-0.5)
    sharpen_prob: float = 0.0  # Probability of sharpening
    sharpen_alpha: float | list[float] = [0.2, 0.5]  # Sharpening strength
    random_shadow_prob: float = 0.0  # Probability of adding random shadows
    random_shadow_intensity: float | list[float] = [0.3, 0.7]  # Shadow intensity
    
    # === Resize (keep at the end of pipeline) ===
    ...
    def get_empty() -> Self:
        return Augmentations(
            ...
            hsv_prob=0.0,
            jpeg_quality=100,
            video_compression_prob=0.0,
            downscale_prob=0.0,
            gaussian_noise_sigma=0.0,
            iso_noise_prob=0.0,
            multiplicative_noise_prob=0.0,
            coarse_dropout_prob=0.0,
            grid_mask_prob=0.0,
            cutmix_prob=0.0,
            real_fake_mixup_prob=0.0,
            auto_augment_prob=0.0,
            resize=None,
        )
...
class Config(Validation, validate_assignment=True):
    ...
    val_split: float = 0.2  # Validation split ratio (0.0 = no validation split)
    ...
    use_balanced_sampling: bool = False  # Enable balanced sampling for training
    sampling_strategy: Literal["weighted", "class_balanced", "stratified", "source_balanced"] = "weighted"
    # - "weighted": WeightedRandomSampler (Real/Fake 클래스별 가중치 부여, 추천)
    # - "class_balanced": ClassBalancedSampler (Real/Fake 클래스별 동일 샘플 수)
    # - "stratified": StratifiedBatchSampler (배치별 Real/Fake 균형)
    # - "source_balanced": 생성 모델별 균형 (SiT, DiT, PixArt 등 소스별 균형) ⭐ NEW
