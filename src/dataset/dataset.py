...
class DeepfakeDataset(BaseDataset):
    ...
    def __init__(
        ...
        split: str = "full",  # "full", "train", "val"
        val_split: float = 0.2,  # Validation split ratio
        split_seed: int = 42,  # Seed for reproducible splitting
    ):
        files = []
        labels = []
        ...
        # 소스별로 고유 ID 매핑 (메모리 효율)
        source_names = []  # 소스 이름 (문자열)
        source2id = {}  # 소스 이름 → ID 매핑
        source_ids = []  # 각 샘플의 소스 ID (정수)
        ...
        for path in files:
            ...
            if source not in source2id:
                source2id[source] = len(source_names)
                source_names.append(source)
            source_ids.append(source2id[source])
        ...
        # source_names와 source2id 저장 (나중에 복원용)
        self.source_names = source_names
        self.source2id = source2id

        logger.print_info("Files loaded")
        
        # Apply train/val split if specified
        if split in ["train", "val"]:
            ...
            split_idx = int(n_files * (1 - val_split))
            ...
            source_ids = [source_ids[i] for i in selected_indices]  # 소스 ID도 split
            
            logger.print_info(f"Using {split} split: {len(files)}/{n_files} files ({len(files)/n_files*100:.1f}%)")

        # 정수 ID를 문자열로 변환 (BaseDataset용)
        sources = [source_names[sid] for sid in source_ids]
        super().__init__(files, labels, preprocess, augmentations, shuffle, dataset2files, sources)
    ...
    def get_source_from_file(file_path: str) -> str:
        ...
        # Check if path contains "real" folder or real data indicators
        if "/real/" in file_path or "original_sequences" in file_path or "youtube" in file_path:
            return "real"
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            image = Image.open(path)
            # Convert to RGB to handle RGBA/P/L mode images
            if image.mode != 'RGB':
                image = image.convert('RGB')
        except Exception as e:
            logger.print_warning(f"Corrupted image file: {path}, error: {e}")
