...
class GenD(BaseDeepakeDetectionModel):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Check if CutMix or Real-Fake Mixup was applied
        if (hasattr(batch, 'cutmix_applied') and batch.cutmix_applied) or \
           (hasattr(batch, 'mixup_applied') and batch.mixup_applied):
            # CutMix or Mixup: mixed loss for two labels
            loss_inputs_a = LossInputs(
                logits_labels=outputs.logits_labels,
                labels=batch.labels,
                l2_embeddings=outputs.l2_embeddings,
            )
            loss_inputs_b = LossInputs(
                logits_labels=outputs.logits_labels,
                labels=batch.label_b,
                l2_embeddings=outputs.l2_embeddings,
            )
            
            loss_a = self.criterion(loss_inputs_a)
            loss_b = self.criterion(loss_inputs_b)
            
            # Weighted combination based on lambda
            lam = batch.lam
            loss = LossOutputs(
                ce_labels=(loss_a.ce_labels * lam + loss_b.ce_labels * (1 - lam)) if loss_a.ce_labels is not None else None,
                uniformity=loss_a.uniformity,  # Uniformity doesn't depend on labels
                alignment_labels=(loss_a.alignment_labels * lam + loss_b.alignment_labels * (1 - lam)) if loss_a.alignment_labels is not None else None,
                total=loss_a.total * lam + loss_b.total * (1 - lam),
            )
        else:
            # Normal loss
