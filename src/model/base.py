...
class Batch:
    ...
    # CutMix fields
    label_b: None | torch.Tensor = None
    lam: float = 1.0
    cutmix_applied: bool = False
    # Real-Fake Mixup fields
    mixup_applied: bool = False
    ...
    def from_dict(batch: dict):
        ...
        return Batch(
            ...
            lam=batch.get("lam", 1.0),
            cutmix_applied=batch.get("cutmix_applied", False),
            mixup_applied=batch.get("mixup_applied", False),
        )
...
class BaseDeepakeDetectionModel(pl.LightningModule):
    ...
    def load_checkpoint(self, checkpoint: str):
        if checkpoint:
            # Check if checkpoint is a directory (HuggingFace format)
            import os
            if os.path.isdir(checkpoint):
                # Try to load HuggingFace model
                try:
                    from src.hf.modeling_gend import GenD as GenDHF
                    logger.print(f"Loading HuggingFace model from {checkpoint}")
                    hf_model = GenDHF.from_pretrained(checkpoint)
                    state_dict = hf_model.state_dict()
                    incompatible_keys = self.load_state_dict(state_dict, strict=False)
                    self.print_checkpoint_keys(incompatible_keys)
                    return
                except Exception as e:
                    logger.print_warning(f"Failed to load as HuggingFace model: {e}")
                    logger.print_warning("Skipping checkpoint loading")
                    return
