...
def main(config: Config, train: bool):
    ...
    # Disable validation if val_split is 0
    num_sanity_val_steps = 0 if config.val_split == 0.0 else 2
    limit_val_batches = 0 if config.val_split == 0.0 else config.limit_val_batches
    ...
    trainer = Trainer(
        ...
        num_sanity_val_steps=num_sanity_val_steps,  # Disable sanity check if no validation
